...
async def verify_with_gemini(image_bytes):
    ...
    payload = {
        "contents": [{
            "parts": [
                ...
                {"text": "Does this image show a YouTube channel page with a Subscribed button? The button might say Subscribed with a bell icon or dropdown arrow. If you can see the word Subscribed anywhere return valid true else false. Respond only in JSON like this: {valid: true, reason: found} or {valid: false, reason: not found}"}
            ]
        }]
    }
    ...
    if match:
        try:
            ...
            pass
    if "true" in text.lower() or "subscribed" in text.lower():
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🎉 Namaskar!\n\n{YOUR_CHANNEL_NAME} ko subscribe karo aur screenshot bhejo — special link milega!\n\n⚠️ Link sirf 15 second tak rahega!",
    ...
    try:
        ...
        if result.get("valid"):
            ...
            await msg.reply_text(
                f"❌ Subscribe nahi dikh raha!\n\nPehle {YOUR_CHANNEL_NAME} ko subscribe karo phir clear screenshot bhejo.",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
